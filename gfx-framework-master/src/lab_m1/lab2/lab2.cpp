...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(-1, -1,  2), glm::vec3(1, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, -1,  2), glm::vec3(1, 1, 0), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(-1, 0,  2), glm::vec3(1, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
            VertexFormat(glm::vec3(0, 0,  2), glm::vec3(0, 0, 0), glm::vec3(0.2, 0.8, 0.6)),
    ...
    {
        VertexFormat(glm::vec3(0, 0, 2), glm::vec3(1, 0, 0), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(0, 0, 4), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(2, 0, 3), glm::vec3(0, 0, 1), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(1, 2, 3), glm::vec3(1, 1, 0), glm::vec3(0.2, 0.8, 0.6)),
    };
    ...
    CreateMesh("tetraedru_C", t_vertices, t_indices);

    // TODO(student): Create a square using two triangles with
    // opposing vertex orientations.
    vector<VertexFormat> s_vertices
    {
        VertexFormat(glm::vec3(4, 0, 0), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(8, 0, 0), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(4, 4, 0), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)),
    ...
    CreateMesh("square_D", s_vertices, s_indices);

    circle_vertex_no = 90;
    vector<VertexFormat> circlePoints;
    circlePoints.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6)));
    float step = 360.0f / circle_vertex_no;
    float measure = 0;
    for (int i {1}; i <= circle_vertex_no; ++i) {
        double x = 2 * cos(measure * PI / 180);
        double y = 2 * sin(measure * PI / 180);
        VertexFormat tmp(glm::vec3(x, y, 0), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6));
        circlePoints.push_back(tmp);
        measure += step;
    }

    // sphere personal task
    circles_no = 720;
    float step2 = 360.0f / circles_no;
    float measure2 = step2;
    for (int j {1}; j < circles_no; ++j) {
        for (int i{ 1 }; i <= circle_vertex_no; ++i) {
            float old_x = circlePoints[i].position.x;
            float old_y = circlePoints[i].position.y;
            float old_z = circlePoints[i].position.z;
            double x = old_x * cos(measure2 * PI / 180) - old_z * sin(measure2 * PI / 180);
            double z = old_x * sin(measure2 * PI / 180) + old_z * cos(measure2 * PI / 180);
            VertexFormat tmp(glm::vec3(x, old_y, z), glm::vec3(0, 1, 0), glm::vec3(0.2, 0.8, 0.6));
            circlePoints.push_back(tmp);
    ...
    vector<unsigned int> c_indices;
    int a = 1;
    int b = 2;
    for (int j{ 0 }; j < circles_no; ++j) {
        for (int i{ 1 }; i <= circle_vertex_no; ++i) {
            if (i == circle_vertex_no) {
                c_indices.push_back(0);
                c_indices.push_back(a);
                c_indices.push_back(1 + j * circle_vertex_no);
                ++a;
                ++b;
                break;
            }
    ...
    CreateMesh("circle", circlePoints, c_indices);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(2, 0, 0), glm::vec3(0.25f));

    // TODO(student): Draw the tetrahedron
    // RenderMesh(meshes["tetraedru_C"], shaders["VertexColor"], glm::vec3(0, 0, 2), glm::vec3(0.25f));

    // TODO(student): Draw the square
    // RenderMesh(meshes["square_D"], shaders["VertexColor"], glm::vec3(4, 0, 0), glm::vec3(0.25f));
