...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        lightPosition = glm::vec3(0, 1, -1);
        lights[1] = lightPosition;
        lightDirections[1] = lightDirection;
...
void Lab8::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, lights[0]);
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, lights[1]);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform3fv(light_position, 2, glm::value_ptr(lights[0]));

    int light_direction = glGetUniformLocation(shader->program, "light_direction");
    glUniform3fv(light_direction, 2, glm::value_ptr(lightDirections[0]));
    ...
    int spotlight = glGetUniformLocation(shader->program, "spotlightOn");
    glUniform1i(spotlight, spotlightOn);

    int spotRad = glGetUniformLocation(shader->program, "spotRadius");
    glUniform1f(spotRad, spotRadius);

    int dir = glGetUniformLocation(shader->program, "spotDirection");
    glUniform3f(dir, spotDirection.x, spotDirection.y, spotDirection.z);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        spotDirection = glm::vec3(cos(RADIANS(x)), -1, sin(RADIANS(z)));
    }
}
