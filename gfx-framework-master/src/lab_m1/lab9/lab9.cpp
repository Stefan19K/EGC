...
void Lab9::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, mapTextures["earth"]);
    }
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    /*for (int i{}; i < height; ++i) {
        for (int j{}; j < width * channels; j += 3) {
            data[i * width + j] = (int)color.x;
            data[i * width + j + 1] = (int)color.y;
            data[i * width + j + 2] = (int)color.z;
        }
    }*/
